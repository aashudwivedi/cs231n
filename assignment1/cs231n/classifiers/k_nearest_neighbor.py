...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    product_term = -2 * np.dot(X, self.X_train.T)
    dists = X_sq.reshape(X_sq.shape[0], 1) + X_train_sq - product_term
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      closest_idxs = np.argsort(dists[i])
      closest_y = self.y_train[closest_idxs]
      # print('{} -- {} -- {}'.format(i, closest_idxs, dists[i]))
